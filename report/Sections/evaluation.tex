...
We evaluate three logical datasets: \texttt{dataset\_init.csv}, \texttt{dataset.csv}, and \texttt{dataset\_filter.csv}. The first file uses an older metadata schema, so we convert its duration, passing-test count, and failing-test count fields into the corresponding current schema fields. Because it has no stage field, we assign an unknown stage label. For all datasets, the binary target is
\[
